...
GetOptions('h' => \(my $help = ''),
	'p=s' => \(my $orthology2pathwayFile = "$fmapPath/FMAP_data/KEGG_Pathway2Level1.txt"),
	'd=s' => \(my $pathwayDefinitionFile = "$fmapPath/FMAP_data/KEGG_module.txt"),
);
if($help || scalar(@ARGV) == 0) {
	die <<EOF;

Usage:   perl ConvergePathway2Level1.pl all.merged.abundance.KeepID.Pathway.txt > all.merged.abundance.KeepID.Pathway.Level1.txt
