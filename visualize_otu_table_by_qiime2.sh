...
<<COMMENT2
	...
	qiime diversity core-metrics --i-table ${output_dir}/${sample_name}.count.qza  --p-sampling-depth $depth --m-metadata-file $mapping_file  --output-dir ${output_dir}/core-metrics-results
	...
	qiime diversity alpha-rarefaction   --i-table ${output_dir}/${sample_name}.count.qza --p-max-depth 10000  --m-metadata-file $mapping_file  --o-visualization ${output_dir}/core-metrics-results/alpha-rarefaction.qzv   --p-steps 50
	...
	echo "##############################################################\n#alpha dviersity summary"
		mkdir alpha
		qiime diversity alpha --i-table ${output_dir}/${sample_name}.count.qza --p-metric chao1 --output-dir ${output_dir}/core-metrics-results/alpha/chao1
		qiime diversity alpha --i-table ${output_dir}/${sample_name}.count.qza --p-metric shannon --output-dir ${output_dir}/core-metrics-results/alpha/shannon
		qiime diversity alpha --i-table ${output_dir}/${sample_name}.count.qza --p-metric observed_otus --output-dir ${output_dir}/core-metrics-results/alpha/observed_otus
