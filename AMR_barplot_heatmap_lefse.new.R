...
#Usage: Rscript ~/pipelines/Github/CII_meta/AMR_barplot_heatmap_lefse.new.R -i ~/Desktop/AMR/all.AMR.abundance.txt -m ~/Desktop/demo.mapping.txt -g Group1
...
#setwd("~/Desktop")
#filename_data<-"all.abundance.txt"
#filename_meta<-"RAO_mapping.txt"
...
dim(data5)
colSums(data5)
AMRID<-rownames(data5)
AMRID_new <- stringr::str_split(AMRID,"\\|") %>% sapply(extract2, 4)
rownames(data5)<-AMRID_new
